(async function () {
  ...
  function renderList() {
    ...
    list.innerHTML = filtered.length ? filtered.map((guide) => `
      <article class="admin-guide">
        ...
        <div class="admin-guide-meta">${P.statusPill(guide.status)}<p style="margin-top:7px">Cadastur: ${P.escapeHTML(guide.cadastur_numero || "Não informado")}</p></div>
  ...
  document.querySelector("#admin-edit-form").addEventListener("submit", async (event) => {
    ...
    if (error) return P.setMessage("#dialog-message", error.message, "error");
    await loadGuides();
    currentGuide = findGuide(currentGuide.id);
    P.setMessage("#dialog-message", "Correções salvas.", "success");
  });
